  ...
  px4__msg__OffboardControlMode__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4__msg__OffboardControlMode * data =
      (px4__msg__OffboardControlMode *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4__msg__OffboardControlMode__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          px4__msg__OffboardControlMode__fini(&output->data[i]);
        }
        return false;
      }
    }
